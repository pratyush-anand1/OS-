...
char *used;				// store which page replacement algorithm to use 
...
//    printf("page fault on page #%d\n",page); // print this virtual page is needed
    ...
    if ( ( (curr_bits & PROT_READ)==0 ) && ( (curr_bits & PROT_WRITE)==0 ) && ( (curr_bits & PROT_EXEC)==0 ) )
    {
		...
		{ 
			...
			else if (!strcmp(used, "fifo"))
			{
				fifo_pra(pt, page);
			}
			
			else if (!strcmp(used, "custom"))
			{
				custom_pra(pt, page);
			}
...
int main( int argc, char *argv[] )
{
	...
	used = argv[3];			// store which page replacement algorithm to use 
    ...
    {
       is_frame_occup_struc[i] = 0;  
       frame_holds_what[i] = 0;  		 
    }


	// if fifo aglo is to be used
	if ( !strcmp(used, "fifo") )
	{
	...
	// initialize pointers to page list (LRU)
	if ( !strcmp(used, "custom") )
	{
		head = NULL;
		tail = NULL;
